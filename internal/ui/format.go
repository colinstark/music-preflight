...
// all counters. When dryRun is true it prefixes a banner clarifying that
...
func formatReport(r core.Report, dryRun bool) string {
	counts := fmt.Sprintf(
		"Renamed: %d\nCovers Resized: %d\nExtracted: %d\nEmbedded Resized: %d\nTranscoded: %d\nGenres Set: %d\nAlbum Artists Set: %d\nTags Edited: %d\nSkipped: %d\nFailed: %d",
		r.Renamed, r.CoversResized, r.Extracted, r.EmbeddedResized, r.Transcoded, r.GenresSet, r.AlbumArtistsSet, r.TagsEdited, r.Skipped, r.Failed,
	)
	if dryRun {
