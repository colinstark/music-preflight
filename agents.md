...
make build-gui                   # GUI app via Wails (needs `wails` CLI + bun)
make gui-frontend                # rebuild the committed GUI bundle (needs bun)
make gui-dev                     # GUI live-reload dev session (needs `wails` + bun)
...
bun and no Wails CLI** — the built GUI bundle is committed under
`cmd/coverfixer-gui/frontend/dist/`, and `internal/ui` is tested headless. Only
`make gui-dev`, `make build-gui`, and `make gui-frontend` need bun.

### Front-end conventions (Wails GUI)
- The GUI is **Svelte 5 + Vite** (bun-managed), source under
  `cmd/coverfixer-gui/frontend/src/`. Vite builds to the committed
  `cmd/coverfixer-gui/frontend/dist/` which Wails embeds via
  `//go:embed all:frontend/dist`. Edit the `.svelte`/`.ts` sources and rebuild
  with `make gui-frontend` (or `wails dev` for live reload).
- A single runes store (`src/lib/store.svelte.ts`) owns the view state machine
  (`phase: empty | loading | ready`, plus an orthogonal `running` flag), the
  run-lifecycle wiring (`cf:*` events), the sidebar form, and the staged
  per-album edits. Components are split into `primitives/` (Button, Toggle,
  TabBar, TextField), `partials/` (OptionGroup, AlbumCard, AlbumHeader,
  TrackRow), and `blocks/` (TitleBar, EmptyState, FolderBar, Sidebar,
  ActionBar, Output, Library, EditAlbumModal). Pure helpers (`prepareAlbum`,
  `discGroups`, `editsEqual`, …) live in `src/lib/preview.ts`, free of DOM and
  store coupling.
...
- Engine → frontend streaming uses `runtime.EventsEmit` over four events:
  ...
  `cf:state` (bool running), plus `cf:folder` (dir, emitted by `app.go` for
  window/Dock drops). Keep these names stable; they are part of the
  `internal/ui` contract (`emitter.go`).
- The frontend calls the Wails-injected `window.go.main.App` /
  `window.runtime` globals directly via a typed wrapper
  (`src/lib/wails.ts`); it does **not** import the generated
  `frontend/wailsjs/` bindings (that folder is gitignored, regenerated by
  `wails build`/`wails dev`). So `bun run build` works standalone.
